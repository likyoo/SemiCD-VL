...
class DLV3PHead(BaseDecodeHead):
    ...
    def __init__(self, c1_in_channels, c1_channels, dilations, img_size, vl_sup=False, **kwargs):
        ...
        if self.vl_sup:
            self.conv_seg_for_vl =  nn.Sequential(
                nn.Conv2d(fuse_channels, 256, 3, padding=1, bias=False),
                nn.BatchNorm2d(256),
                nn.ReLU(True),
                nn.Conv2d(256, 256, 3, padding=1, bias=False),
    ...
    def forward(self, inputs, need_contrast=False):
        ...
        out_fea = torch.cat([c1, c4], dim=1)
        out = self.head(out_fea)
