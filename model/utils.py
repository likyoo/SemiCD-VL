...
def bcl_loss(
    ...
    **kwargs):
    ...
    target = target * mask
    utarget = (1 - target) * mask

    if reduction == 'none':
        return utarget * torch.pow(pred, 2) * mask + target * torch.pow(torch.clamp(margin - pred, min=0.), 2)
    elif reduction == 'mean':
        n_u = utarget.sum() + eps
        n_c = target.sum() + eps
...
class BCLLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss = self.loss_weight * bcl_loss(
            pred, target, margin=self.margin, ignore_index=self.ignore_index, reduction=reduction)
        return loss
